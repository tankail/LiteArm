 ...
 * @brief 双臂老化测试程序
 *
 * 让左右手臂同时在起始关节角度和终点关节角度之间来回运动，不停循环
 *
 * 使用方法：
...
// 打印目标角度
void printTargetAngles(const std::vector<double>& angles, const std::string& arm_name)
{
    std::cout << "[" << arm_name << "] 目标角度(°): [";
...
int main(int argc, char** argv)
{
    try {
        ...
        // 速度保持（1.5 rad/s），力矩增大（80 Nm）确保有足够动力完成动作
        std::vector<double> velocity = {1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5};
        std::vector<double> max_torque = {80.0, 80.0, 80.0, 80.0, 80.0, 80.0, 80.0};
        ...
        // 先让手臂运动到起始位置
        std::cout << "\n正在移动到起始位置..." << std::endl;
        printTargetAngles(right_start, "右手臂");
        printTargetAngles(left_start, "左手臂");
        ...
        while (keep_running) {
            ...
            if (go_to_end) {
                ...
                printTargetAngles(right_end, "右手臂");
                printTargetAngles(left_end, "左手臂");
        ...
        // 停止时发送最后一次位置命令，确保手臂保持在当前位置
        std::cout << "保持手臂当前位置..." << std::endl;
